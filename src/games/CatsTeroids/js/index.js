...
function draw() {
    ...
    if (menu == false) {
        if (paused == false) {
            ...
            // followMouse
            if(mouse.x > player.pos.x){
            player.rot = Math.atan((mouse.y - player.pos.y)/(mouse.x - player.pos.x))
            }else{
                player.rot = (Math.atan((mouse.y - player.pos.y)/(mouse.x - player.pos.x))) + Math.PI
            }
            ...
            if (level % 5 == 0) {
                ...
                if (boss.life <= 20 && !(boss.nome === "Gojo Satoru")) {
                    boss.life = 0;
                    bossMode = false;
        ...
        if (ctrl_screen) {
            ...
            ctx.fillText('Rotate Player: Mouse', 400, 300);
            ctx.fillText('Move Forward: W', 340, 400);
            ctx.fillText('Shoot Projectile: Spacebar / L Click', 600, 500);
